...
class NpyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Resize to 1024×1024 (SAM ViT-B expects 1024×1024)
        gray_resized = transform.resize(
            gray, (512, 512), order=1, preserve_range=True, anti_aliasing=True
        ).astype(np.float32)
        mask_resized = transform.resize(
            gt,   (512, 512), order=0, preserve_range=True, anti_aliasing=False
        )
...
def main():
    ...
    # force batch_size=1 on MPS to avoid OOM
    ...
    val_ds   = NpyDataset(args.val_images,   args.val_masks)   # <-- fixed typo here
    ...
    # prepare full-image box
    sample_img,_ = train_ds[0]
    _,H,W       = sample_img.shape  # (3, H, W)
    ...
    for epoch in range(args.epochs):
        model.train()
        running_loss = 0.0
        # wrap train_loader in tqdm to show batch progress
